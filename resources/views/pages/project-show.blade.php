...
@section('content')
  <style>
    ...
    .page-project * { box-sizing: border-box }

    /* ---------- HERO (left-aligned + long pill breadcrumb) ---------- */
    .pr-hero { background: var(--navy); color: #fff; padding: 78px 0 92px; position: relative; }
    .pr-hero .pr-hgroup { max-width: 1100px; margin: 0 auto; padding: 0 12px }
    .pr-title { font-size: 48px; line-height: 1.08; font-weight: 800; margin: 0 0 10px }
    @media (min-width:992px){ .pr-title{ font-size:56px } }
    .pr-hero h1,.pr-hero .pr-title,.pr-hero p,.pr-hero .pr-sub { color:#fff !important }
    .pr-sub { max-width: 680px; font-size: 15px; line-height: 1.7; margin: 0; opacity: .95 }

    .pr-bread-wrap { position:absolute; left:0; right:0; bottom:-28px; display:flex; justify-content:center }
    .pr-bread { width:min(1180px, calc(100% - 48px)); background: var(--sky); height:46px; border-radius:9999px;
      display:flex; align-items:center; gap:18px; padding:0 22px; font-weight:700; box-shadow:0 10px 18px rgba(3,102,140,.12); color:#fff !important; }
    .pr-bread a,.pr-bread span { color:#fff !important }
    .pr-bread .sep { color:rgba(255,255,255,.85) !important }
    .pr-bread .home-ico { display:inline-grid; place-items:center; width:26px; height:26px; border-radius:50%; background:rgba(255,255,255,.22); color:#fff !important; font-size:12px }

    /* ---------- Content ---------- */
    .pr-wrap { padding: 70px 0 60px }
    ...
    /* Media */
    .media-card { background: var(--card); border:1px solid #eef2f6; border-radius:18px; overflow:hidden; box-shadow:var(--shadow); }
    .media-thumb { position:relative; aspect-ratio:4/3; background:#f2f4f8 }
    .media-thumb img { width:100%; height:100%; object-fit:cover; display:block }
    .media-placeholder { display:grid; place-items:center; aspect-ratio:4/3; background:#f8fafc; color:#94a3b8 }

    /* Text */
    .pr-body-card { background:var(--card); border:1px solid #eef2f6; border-radius:18px; box-shadow:var(--shadow); padding:22px; }
    @media (min-width:992px){ .pr-body-card{ padding:28px } }
    .pr-h2 { font-weight:800; color:#0f172a; margin:0 0 8px }
    .pr-lead { color:#0ea5e9; margin:0 0 12px; font-weight:700 }
    .prose { color:#1f2937; line-height:1.75; font-size:1.05rem }
    .prose p { margin-bottom:1rem }

    /* Buttons */
    .btn-accent{ background:var(--accent); color:#fff; border:none; font-weight:800; padding:12px 18px; border-radius:14px; }
    .btn-accent:hover{ filter:brightness(.98) }
    .btn-outline-accent{ background:#fff; color:#0f1e3d; border:1px solid #dbe4f0; font-weight:700; padding:12px 18px; border-radius:14px; }
    .btn-outline-accent:hover{ background:#f8fafc }

    /* Split hero (aligné avec Services) */
    .sv-hero--split{ --sv-img-drop:160px; margin-bottom:var(--sv-img-drop); }
    .sv-hero__inner{ position:relative; z-index:3; }
    .sv-hero::after{ content:""; position:absolute; inset:0; background:var(--navy); z-index:2; pointer-events:none; }
    .sv-hero__media{ position:absolute; right:var(--sv-img-right,24px); bottom:calc(-1 * var(--sv-img-drop,160px)); width:var(--sv-img-w,560px);
      height:var(--sv-img-h,380px); border-radius:var(--sv-img-radius,22px); overflow:hidden; background:transparent; z-index:1; }
    .sv-hero__media img{ width:100%; height:100%; display:block; object-fit:cover; border:0; box-shadow:none!important; filter:none!important; transform:none!important; }
    .sv-hero{ position:relative; background:var(--navy); color:#fff; padding:var(--sv-hero-pad,140px) 0; overflow:visible; z-index:5; }
    @media (max-width: 992px){ .sv-hero__media{ position:static; width:100%; height:var(--sv-img-h-mobile,280px); right:auto; bottom:auto; margin-top:18px; } }
    .page-service .sv-hero__copy{ position:relative; transform:translate(-52px, -11px)!important; will-change:transform; }
    @import url('https://fonts.googleapis.com/css2?family=Epilogue:wght@400;600;800;900&display=swap');

    /* HERO projet */
    .pr-hero{ position:relative; background:var(--navy); color:#fff; padding:var(--pr-hero-pad,160px) 0; overflow:visible; z-index:5; }
    .pr-hero::after{ content:""; position:absolute; inset:0; background:var(--navy); z-index:2; pointer-events:none; }
    .pr-hero--split{ --pr-img-drop:220px; margin-bottom:var(--pr-img-drop); }
...
<section class="clean-energy my-5">
  <div class="container">
    ...
    <p class="mb-4 text-muted">
      ...
      <strong>tertiaire, de l’industrie et du secteur agricole</strong> dans l’optimisation de la performance énergétique
    ...
    <div class="row mb-4">
      <div class="col-md-6">
        <ul class="list-unstyled">
          <li><i class="fas fa-check-circle text-primary me-2"></i> Isolation de réseaux hydrauliques de chauffage pour limiter les pertes de chaleur.</li>
          <li><i class="fas fa-check-circle text-primary me-2"></i> Installation et calorifugeage des points singuliers.</li>
          <li><i class="fas fa-check-circle text-primary me-2"></i> Isolation des combles pour un confort thermique optimal été comme hiver.</li>
          <li><i class="fas fa-check-circle text-primary me-2"></i> Isolation des planchers bas pour réduire les déperditions énergétiques.</li>
        </ul>
      </div>
      <div class="col-md-6">
        <ul class="list-unstyled">
          <li><i class="fas fa-check-circle text-primary me-2"></i> Installation de déstratificateurs d’air pour homogénéiser la température ambiante.</li>
          <li><i class="fas fa-check-circle text-primary me-2"></i> Systèmes de régulation pour groupes froids à haute pression flottante.</li>
          <li><i class="fas fa-check-circle text-primary me-2"></i> Intégration de déshumidificateurs d’air pour un contrôle optimal de l’humidité.</li>
          <li><i class="fas fa-check-circle text-primary me-2"></i> Accompagnement complet de la conception à la mise en œuvre des solutions énergétiques.</li>
        </ul>
      </div>
    </div>
    ...
    <div class="row text-center">
      <div class="col-md-4 mb-4">
        ...
        <p class="text-muted">
          Isolation et régulation des installations industrielles pour optimiser les rendements et réduire les pertes énergétiques.
        </p>
      </div>
      <div class="col-md-4 mb-4">
        <div class="icon mb-3">
        ...
        <p class="text-muted">
          Solutions d’isolation et de confort thermique pour les bâtiments de bureaux, établissements publics et infrastructures collectives.
        </p>
      </div>
      <div class="col-md-4 mb-4">
        <div class="icon mb-3">
        ...
        <p class="text-muted">
          Amélioration de la performance énergétique des exploitations agricoles, serres et bâtiments d’élevage grâce à des solutions adaptées.
        </p>
      </div>
    </div>
  </div>
