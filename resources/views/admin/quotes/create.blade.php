            ...
            <div class="mb-3">
                ...
                <select name="secteur" id="secteur" class="form-select @error('secteur') is-invalid @enderror" required>
                    ...
                    @foreach($secteurs as $s)
                        <option value="{{ $s }}" {{ old('secteur') === $s ? 'selected' : '' }}>
                            {{ ucfirst($s) }}
                        </option>
                    @endforeach
                </select>
...
<script>
    ...
    const initialSecteur = "{{ old('secteur') }}";
    const oldOps = @json($opsSelected);
    const oldQS = @json($oldQS);

    const form          = document.getElementById('admin-quote-create');
    ...
    // Helper pour cocher radio à partir de old('qs')
    ...
    // Maj des opérations selon secteur
    function updateOperations() {
        ...
        if (secteur && secteurOps[secteur]) {
            ...
            // Si on revient d'une validation échouée, réafficher les questionnaires
            if (oldOps.length) {
